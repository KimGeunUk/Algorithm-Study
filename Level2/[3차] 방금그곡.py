...
def solution(m, musicinfos):
    ...
    for music in musicinfos:
        infos = music.split(',')
        s, e, title, sheet = infos[0], infos[1], infos[2], infos[3]
        ...
        len_sheet = len(sheet)
        len_shop = sheet.count('#')
        d_,m_ = divmod(playtime, len_sheet-len_shop)
        sheet = sheet * d_ + sheet[:m_+sheet[:m_+len_shop].count('#')]

        if m in sheet:
            m_isin = False
            if len(sheet) > len(m):
                for i in range(len(sheet) - len(m)):
                    if sheet[i:i+len(m)] == m:
                        if sheet[i+len(m)] != '#':
                            m_isin = True
                            break
                
            elif len(sheet) == len(m) and sheet == m:
                m_isin = True
            
            if m_isin == True:
                if len(answer) == 0:
                    answer = [title, playtime]
                elif answer[1] < playtime:
                    answer = [title, playtime]
                elif answer[1] == playtime:
                    continue
...
print(solution("ABCD", ["12:00,12:14,HI,CDEFGAB", "12:00,12:14,HELLO,CDEFGAB", "13:00,13:05,WORLD,ABCDEF"])) # "HELLO"
